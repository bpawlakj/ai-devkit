...
if $DO_CLAUDE; then
    ...
    echo "    Skills:   $(find "$CLAUDE_DIR/skills" -mindepth 1 -maxdepth 1 -type d 2>/dev/null | wc -l | tr -d ' ') (kickoff, discover, product-spec, research, save-plan, atomize, implement, agents-md, rule-review, open-web, bitbucket-review)"
...
if $DO_COPILOT; then
    ...
    echo "    Prompts:  15 templates (ship, retro, changelog, threat-model, init-permissions [shell-script docs], kickoff, discover, product-spec, research, save-plan, atomize, implement, agents-md, rule-review, bitbucket-review)"
    echo ""
fi
