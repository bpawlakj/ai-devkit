...
`/save-plan` detects roadmap-shape automatically (heuristic in Step 1.5) and lands it at `docs/foundation/roadmap.md` instead of inside `docs/work/`. The user can always override the auto-classification at the prompt.
...
1. **One roadmap per project.** It lives at `docs/foundation/roadmap.md`. Versioned overwrites (`roadmap-v2.md`, `roadmap-v3.md`) are the only allowed shape for replacement — never delete an old roadmap.
...
4. **Change ID = `/save-plan` slug.** When a slice goes from `ready` to `in-progress`, the developer runs `/save-plan <change-id>` (or `/plan` then `/save-plan`) and the resulting `docs/work/<NNN>-<change-id>/plan.md` references the slice ID in its front-matter. That's how the bottom-up `docs/work/ROADMAP.md` (auto-generated rollup) reconnects to the top-down `docs/foundation/roadmap.md`.

5. **Two ROADMAP files exist on purpose.**
   - `docs/foundation/roadmap.md` — top-down planning artifact (written by `/save-plan` when roadmap-shape detected; edited in place as the project evolves).
   - `docs/work/ROADMAP.md` — bottom-up derived rollup (auto-regenerated by `regenerate-roadmap.sh` from initiative folders + T-*.md frontmatter).

   Don't conflate them. The first answers "where are we going"; the second answers "where are we now".
