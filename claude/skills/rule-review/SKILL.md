...
name: rule-review
description: >
  Audit an existing agent-rules file (AGENTS.md, CLAUDE.md, .cursor/rules/*.mdc,
  .github/copilot-instructions.md, .github/instructions/*.md) across seven
  dimensions and emit a verdict per finding with concrete evidence. Goes
  beyond length / embedded code / language precision / redundancy / order
  (which 10x covers) by adding (6) cross-tool drift detection when multiple
  rules files exist for the same scope, and (7) dead-rule check that greps the
  codebase for the patterns each rule targets — if nothing matches, the rule
  may be obsolete. Cites the ai-devkit rule in ~/.claude/rules/ that already
  covers redundant content. Read-only by default; --fix applies safe rewrites
  (remove redundancy, split oversized rules, reorder by importance).
  Trigger phrases: "review AGENTS.md", "audit my CLAUDE.md", "check my rule
  file", "rule review", "is my AGENTS.md any good".
