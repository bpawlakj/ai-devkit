# Product Spec Schema (canonical reference)

This doc is the single source of truth for the shape of `docs/product-spec.md` produced by `/product-spec`, and for the `checkpoint:` block written into `docs/discover-notes.md` by `/discover`. Both skills load this file by relative path (`skills/discover/references/product-spec-schema.md`) and conform to it.

Two contracts live here:

1. **Spec frontmatter + required sections (10 greenfield / 11 brownfield)** — what `product-spec.md` must contain.
2. **discover-notes.md checkpoint format** — what `discover-notes.md` must carry so a session can resume from disk.

Renames or restructurings of either contract are load-bearing. Update this doc *first*, then both skill bodies.

Spec frontmatter captures only **product-level priors** — what the product is, who it's for, when it ships. Fields that depend on team composition, runtime, or deployment target belong to a separate downstream tech-stack-selection step, not to the spec. Frame the spec as the product's identity, not its architecture.

# Frontmatter fields

Every product spec declares this YAML frontmatter block. Field order is suggested, not load-bearing; key names ARE load-bearing.

```yaml
---
project: <string>                    # human-readable project name, e.g. "Recipe Fridge"
version: <integer>                   # 1 for first spec; bumped only when a versioned save lands (product-spec-v2.md → version: 2)
status: <enum>                       # draft | reviewed | locked
created: <YYYY-MM-DD>                # date the spec was first written
...
- **`project`** — short proper-noun name. The skill never invents this; if the user did not name the project, `/discover` asks before `/product-spec` runs.
- **`version`** — starts at `1`. Versioned saves (`product-spec-v2.md`, `product-spec-v3.md`) increment this so downstream tooling can sort by recency.
- **`context_type`** — `greenfield` for new projects built from scratch; `brownfield` for changes to existing systems. Determines which section list (10 greenfield vs 11 brownfield) the spec uses. Set by `/discover`'s auto-detection; `/product-spec` reads it from `discover-notes.md` frontmatter.
- **`status`** — `draft` on first write. `/product-spec` does not promote past `draft`; downstream skills (review, sign-off) flip it.
...
Stack-shaped concerns — team composition, language preferences, technology avoid-lists, deployment mode, region, budget tier, CI/CD shape — are intentionally absent from this frontmatter. They belong to the tech-stack-selection step that runs after `/product-spec` and are gathered there. If `/discover` captures forward-looking notes about them, those notes live in `discover-notes.md` body, not in spec frontmatter or sections.
...
A conforming `product-spec.md` contains a set of `##`-level headings determined by `context_type`. Greenfield uses 10 sections; brownfield uses 11 sections. Section names are the contract — downstream parsers split on them. Subsection (`###`) structure is documented per section below.

Entities, fields, and their lifecycles are intentionally NOT a spec concern. They emerge from Functional Requirements and User Stories (which name the nouns the product manipulates) and are pinned down later — during stack selection and implementation planning. A spec that ships with a column-level data model has already over-committed.

# Greenfield Spec Sections (10 sections)

## Vision & Problem Statement

Two paragraphs max. First paragraph: the **specific** pain (named user, named situation, named cost). Second paragraph: the insight that makes this spec worth writing — what does the team understand that competitors / status-quo solutions don't.
...
If `/discover` ran a Socratic round, each FR may carry a `> Socratic:` blockquote underneath capturing the strongest counter-argument and the user's resolution.
...
**One sentence first.** A single declarative sentence that captures the domain rule that makes this product non-trivial. If you cannot write this sentence, the product is empty CRUD and the spec is hollow.
...
If `/discover` flagged the empty-CRUD anti-pattern and the user accepted the warning, this section still exists with the user's chosen domain rule. If they overrode without choosing one, the section reads `# TODO: domain rule — see Open Questions` and `## Open Questions` carries an entry naming the gap.
...
Numbered list. Each entry names what's unknown, who needs to resolve it, and the latest acceptable resolution date if any. The `/product-spec` skill ALWAYS routes captured uncertainty here verbatim — it never invents domain decisions to fill gaps.

```markdown
1. **What is the one-sentence business rule?** — TBD by user. Block: yes (spec is hollow until resolved).
2. **What's the canonical recipe data source?** — Owner: user. By: 2026-05-26.
```

# Brownfield Spec Sections (11 sections)

When `context_type: brownfield`, the spec uses delta-framing: sections describe what changes, not the full system. The brownfield template replaces the 10 greenfield sections with 11 sections optimized for describing changes to an existing system.
...
# discover-notes.md checkpoint format

`/discover` writes `docs/discover-notes.md` with a frontmatter `checkpoint:` block that drives resume behavior. The block's shape is fixed; the body of `discover-notes.md` (the captured discussion, draft FRs, draft user stories) is freeform and may evolve.

```yaml
---
project: <string|null>                     # mirrors spec frontmatter; null until phase 1 lands
context_type: <enum>                       # greenfield | brownfield — set during Step 0.7, load-bearing for /product-spec routing
created: <YYYY-MM-DD>
...
checkpoint:
  ...
  frs_drafted: <integer>                   # count of FR-NNN entries currently in discover-notes.md
...
- `8` — finalized; ready for `/product-spec`.

`quality_check_status` semantics:

- `pending` — cross-check not yet run.
- `warned` — cross-check ran, gaps surfaced, user accepted override. The body lists gaps under `## Quality cross-check` so `/product-spec` can mirror them into `## Open Questions`.
- `accepted` — cross-check ran, no gaps.

On re-entry, `/discover` reads `current_phase` and `phases_completed` and resumes at the next unfinished phase. The skill MUST NOT replay completed phases on resume — only summarize them.

# How `/discover` and `/product-spec` use this doc

- `/discover` auto-detects context type (greenfield vs brownfield) and writes `context_type` into discover-notes.md frontmatter. It produces `discover-notes.md` whose body anticipates the spec sections (10 for greenfield, 11 for brownfield); it captures decisions in the order the spec will need them. For brownfield, the body includes `## Current System` and `## Constraints & Preserved Behavior` sections. It writes the `checkpoint:` block per the format above and validates `quality_check_status` is set before signaling done. Forward-looking notes for downstream chain steps (tech-stack-selection / stack-assessment, longer-term technical roadmap) may live in `discover-notes.md` body under explicitly-named blocks — but they are NOT part of the spec schema and do not flow into spec frontmatter or sections.
- `/product-spec` reads `discover-notes.md` (or a user-supplied notes file), determines `context_type` from frontmatter (or auto-detects from cwd if absent), maps captured content into the correct section list (10 greenfield / 11 brownfield) in order, and routes any captured gaps verbatim into `## Open Questions`. Frontmatter fields it cannot fill from input become `# TODO: <field-name> — see Open Questions` placeholders, and a matching numbered entry lands in `## Open Questions`. Content that sits outside the spec schema (forward-looking notes for downstream chain steps) is summarized into the hand-off message — it is NOT copied into the spec.

# Drift detection

If a maintainer renames a spec section or restructures frontmatter, the failure mode is silent drift between this doc and the skills. The mitigation:

1. Edit this doc first.
2. Grep both skill bodies (`skills/discover/SKILL.md`, `skills/product-spec/SKILL.md`) for the old name; update.
3. Re-validate the skills.
