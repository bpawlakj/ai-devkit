...
The pattern is **facilitator stance + gray-area surfacing + recommended-answer + Socratic challenge**:
...
2. **Surface 3–5 gray areas** as multi-select decisions when the user's first attempt has ambiguities. Use AskUserQuestion. Each option is a real position with a tradeoff, not a placeholder.
...
If any of the four is vague ("everyone", "always", "a lot of pain"), challenge with a Socratic prompt: "What would have to be true about this for it to be the wrong problem to solve?" or "Who specifically have you seen experience this in the last month?"
...
- Insight — what does the user know that the status quo doesn't? (use Socratic: "If your idea is obvious, why hasn't this been built?")
...
- Insight — what does the user know about the current system that makes this change non-obvious? (Socratic: "Why hasn't this been done already?")
...
If the answer is anything but N/A, ask one follow-up about role separation: is this a flat user model, or are there roles (e.g., admin / member / guest) that see different things? Socratic: "What's the smallest access model that would still make the MVP useful?"
...
Socratic: "What's the smallest access change that would still make this feature useful without disrupting existing users?"
...
2. Append a `## Timeline acknowledgment` line under the timeline budget block in discover-notes that records: estimated weeks, that the user explicitly accepted the sustained-effort cost, and the date. Format: `Acknowledged on <YYYY-MM-DD>: <N>-week MVP requires sustained dedication; user accepted.`
...
- "What's the blast radius of this change? Which existing features, integrations, or data flows could break?" (Socratic: "What's the thing an existing user would notice first if this change went wrong?")
...
Read back the current `discover-notes.md` and check each of the following elements. For each, mark `present` or `missing/weak`:
...
For each `missing/weak`, **list it by name** with a one-line consequence: "Business Logic: not captured as a one-sentence rule — your spec will be hollow without a domain decision." Generic "your spec has gaps" warnings nullify the gate; do not write them.
...
- If the user pushes to skip a phase ("just generate the spec already"), explain the consequence: missing phases produce hollow spec sections. Then offer to skip with the cost made explicit. The choice is theirs.
